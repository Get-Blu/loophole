...
// Rough fallback when a provider doesn't report real usage (~4 chars per token is the
// standard English approximation). Used so the token counter never gets stuck at 0.
...
const _sendOpenAICompatibleChat = async ({ messages, onText, onFinalMessage, onError, settingsOfProvider, modelSelectionOptions, modelName: modelName_, _setAborter, providerName, chatMode, separateSystemMessage, overridesOfModel, mcpTools }: SendChatParams_Internal) => {
	...
	openai.chat.completions
		...
		.then(async response => {
			...
			// Provider didn't give us real usage — estimate locally so the counter never gets stuck
			if (!tokenUsage) {
...
const sendGeminiChat = async ({
	...
	let lastUsageMetadata: { promptTokenCount?: number; candidatesTokenCount?: number; totalTokenCount?: number } | undefined
	...
	genAI.models.generateContentStream({
		...
		.then(async (stream) => {
			...
			for await (const chunk of stream) {
				...
				// @ts-ignore — Gemini includes cumulative usage on each chunk; keep the latest one
			...
			if (!fullTextSoFar && !fullReasoningSoFar && !toolName) {
				...
				let tokenUsage: import('../../common/sendLLMMessageTypes.js').TokenUsageInfo | undefined = lastUsageMetadata ? {
				...
				if (!tokenUsage) {
					const inputText = (messages as GeminiLLMChatMessage[]).map((m: any) => typeof m === 'string' ? m : JSON.stringify(m)).join(' ');
					const estInput = estimateTokensFromText(inputText);
					const estOutput = estimateTokensFromText(fullTextSoFar);
					tokenUsage = { inputTokens: estInput, outputTokens: estOutput, totalTokens: estInput + estOutput };
				}
