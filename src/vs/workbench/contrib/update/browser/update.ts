...
export class ProductContribution implements IWorkbenchContribution {
	...
	constructor(
		...
		hostService.hadLastFocus().then(async hadLastFocus => {
			...
			const currentVersion = tryParseVersion(productService.loopholeVersion ?? productService.version);
			...
			if (shouldShowReleaseNotes && !shouldShowPostInstallInfo && !environmentService.skipReleaseNotes && releaseNotesUrl && lastVersion && currentVersion && isMajorMinorUpdate(lastVersion, currentVersion)) {
				showReleaseNotesInEditor(instantiationService, productService.loopholeVersion ?? productService.version, false)
					.then(undefined, () => {
						notificationService.prompt(
							severity.Info,
							nls.localize('read the release notes', "Welcome to {0} v{1}! Would you like to read the Release Notes?", productService.nameLong, productService.loopholeVersion ?? productService.version),
							[{
			...
			storageService.store(ProductContribution.KEY, productService.loopholeVersion ?? productService.version, StorageScope.APPLICATION, StorageTarget.MACHINE);
		});
	}
}
			...
			this._register(registerAction2(class SwitchQuality extends Action2 {
					...
					try {
						...
						const res = await dialogService.confirm({
							...
							detail: newQuality === 'insider' ?
								nls.localize('relaunchDetailInsiders', "Press the reload button to switch to the Insiders version of Loophole.") :
								nls.localize('relaunchDetailStable', "Press the reload button to switch to the Stable version of Loophole."),
				...
				private async selectSettingsSyncService(dialogService: IDialogService): Promise<UserDataSyncStoreType | undefined> {
					const { result } = await dialogService.prompt<UserDataSyncStoreType>({
						...
						detail: nls.localize('selectSyncService.detail', "The Insiders version of Loophole will synchronize your settings, keybindings, extensions, snippets and UI State using separate insiders settings sync service by default."),
						buttons: [
							{
