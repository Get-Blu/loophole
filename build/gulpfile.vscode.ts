...
function patchWin32DependenciesTask(destinationFolderName: string) {
	...
	return async () => {
		...
		const deps = (await Promise.all([
			glob('**/*.node', { cwd, ignore: ['extensions/node_modules/@parcel/watcher/**', '**/darwin-*/**', '**/linux-*/**', '**/alpine-*/**', '**/*-darwin/**', '**/*-linux/**'] }),
			glob('**/rg.exe', { cwd }),
...
BUILD_TARGETS.forEach(buildTarget => {
	...
	const [vscode, vscodeMin] = ['', 'min'].map(minified => {
		...
		const destinationFolderName = `Loophole${dashed(platform)}${dashed(arch)}`;
