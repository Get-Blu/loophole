...
export function cleanNodeModules(...rulePaths: string[]): NodeJS.ReadWriteStream {
	// NOTE: rule files are merged into a single exclude/include pass rather than
	// chained as separate cleanNodeModules() calls piped one after another.
	// If chained, a file excluded by an earlier rule file's blanket pattern
	// (e.g. base .moduleignore stripping all platform binaries) is permanently
	// dropped from the stream before a later rule file (e.g. .moduleignore.win32)
	// gets a chance to re-include it via its own `!` rule, since gulp-filter does
	// not pass through non-matching files. Merging ensures re-includes are
	// evaluated against the original, unfiltered file list.
	const rules = rulePaths.flatMap(rulePath => fs.readFileSync(rulePath, 'utf8')
		.split(/\r?\n/g)
		.map(line => line.trim())
