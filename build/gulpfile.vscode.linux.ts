...
function prepareSnapPackage(arch: string) {
	...
	return function () {
		...
		const electronLaunch = gulp.src('resources/linux/snap/electron-launch', { base: '.' })
			...
			.pipe(es.mapSync((file: any) => {
				// Snapcraft's skeleton check requires this to be world-readable AND
				// executable. Git only tracks two file modes (644/755), and this file
				// has previously been committed as non-executable (644) by contributors
				// on platforms/tools that don't preserve the exec bit — which fails
				// `snap pack --check-skeleton` with "bad permissions" at pack time.
				// Force 0755 here so the build can't silently regress on this again.
				if (file.stat) {
					file.stat.mode = 0o755;
				}
				return file;
			}));
		...
		try {
			...
			try {
				...
				const logFiles = fs.readdirSync(logDir)
					.filter(f => f.startsWith('snapcraft-'))
					.sort();
